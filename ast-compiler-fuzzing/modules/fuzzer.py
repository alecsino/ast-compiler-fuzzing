        ...
        try:
            while n_file_found < self.n_threshold:
                    with mp.Pool(self.num_cores) as pool:
                            ...
                            fuzzed_tests = pool.imap_unordered(self.compiler.compile_test, [(test, self.apply(test, mutated_inputs), old_inputs, mutated_inputs, depth, breadth, old_stats) for test, old_inputs, mutated_inputs, depth, breadth, old_stats in list_of_fuzzed_tests])
